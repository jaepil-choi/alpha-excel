"""
Exploratory Data Analysis for DataGuide Price Monthly Data File

This script explores the dataguide_price_monthly.xlsx file to understand
the monthly price fields available.
"""
...
def main():
    """Main EDA workflow for monthly price data."""
    print_section("DataGuide Price Monthly Data - EDA")

    # Define file path
    file_path = Path("data/unprocessed/fnguide/dataguide_price_monthly.xlsx")
    ...
    # Show unique Item Names (monthly price fields)
    print(f"\n[UNIQUE MONTHLY PRICE FIELDS]")
    ...
    if item_col in df.columns:
        ...
        for item in unique_items[:min(5, len(unique_items))]:  # First 5 fields
            print(f"\n  Field: {item}")
            sample_rows = df[df[item_col] == item].head(3)

            # Get sample date columns
            sample_date_cols = date_cols[:5]
            display_cols = ['Symbol', 'Symbol Name'] + sample_date_cols
            display_df = sample_rows[display_cols]
        ...
        print(f"\n  Warning: 'Item Name' column not found!")
    ...
    if 'Frequency' in df.columns:
        ...
        print(f"  'Frequency' column not found")
    ...
    if 'Kind' in df.columns:
        ...
        print(f"  'Kind' column not found")

    # Show first few rows of all metadata
    print(f"\n[First 10 Rows - All Metadata Columns]")
    print(df[metadata_cols].head(10).to_string(index=False))
