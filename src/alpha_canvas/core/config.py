"""
DEPRECATED: ConfigLoader has been replaced by alpha-database.DataSource

This module is no longer used in production code. Config loading is now handled
by alpha-database's independent ConfigLoader.

This code is kept for future reference only. All functionality has been migrated
to alpha-database package.

Migration:
    Old: from alpha_canvas.core.config import ConfigLoader
    New: from alpha_database.core.config import ConfigLoader (in alpha-database)
    
For data loading, use DataSource instead:
